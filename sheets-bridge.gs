  ...
  * 2. Renomeie as abas: "Estabelecimentos", "Transacoes", "Usuarios" (ou "Usuários").
  ...
  */
  ...
  function doGet(e) {
    ...
    const estValues = estSheet ? estSheet.getDataRange().getValues() : [[]];
    ...
    // 2. Get Authorized Users (Flexível com acentuação)
    let userSheet = ss.getSheetByName("Usuarios") || ss.getSheetByName("Usuários");
    let authorizedUsers = [];
    if (userSheet) {
      const userValues = userSheet.getDataRange().getValues();
      authorizedUsers = userValues.slice(1)
        .filter(row => row[1] && String(row[1]).includes("@")) // Garante que tem algo parecido com e-mail
        .map(row => ({
