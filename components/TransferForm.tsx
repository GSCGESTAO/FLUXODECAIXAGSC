...
export const TransferForm: React.FC<TransferFormProps> = ({ establishments, onSave, userEmail }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Descrição enriquecida com o motivo (observação)
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
      <div className="w-full max-w-xl bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col max-h-[95vh] animate-fade-in">
        
        {/* Header Fixo */}
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-amber-500 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-amber-200 dark:shadow-none">
        ...
        {/* Corpo do Formulário com scroll interno se necessário, mas desenhado para caber */}
        <form onSubmit={handleSubmit} className="p-8 space-y-5 overflow-y-auto scrollbar-hide">
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-700 flex gap-3">
              ...
              <button 
                ...
                className={`flex-[2] py-4 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl transition-all active:scale-95 flex items-center justify-center gap-2 ${isSubmitting ? 'bg-slate-400' : 'bg-amber-500 shadow-amber-200 dark:shadow-none hover:bg-amber-600'}`}
              >
