...
export const TransferForm: React.FC<TransferFormProps> = ({ establishments, onSave, userEmail }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
      <div className="w-full max-w-xl bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col max-h-[85vh] animate-fade-in">
        ...
        {/* Formulário com scroll interno para garantir que o rodapé fixo apareça */}
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-hide">
            {/* Visual Flow Compacto */}
            <div className="flex items-center justify-between gap-3 bg-slate-100/50 dark:bg-slate-900/50 p-4 rounded-3xl border border-slate-100 dark:border-slate-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full text-2xl font-black pl-10 pr-4 py-3 border border-slate-200 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-amber-500/10 outline-none bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white" 
                        placeholder="0,00" 
                      />
                    </div>
                </div>
                <div className="space-y-1.5">
                    ...
                    <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none bg-slate-50 dark:bg-slate-900 text-sm font-bold h-[54px]" />
                </div>
            </div>
          ...
          {/* Footer Fixo no Rodapé do Pop-up */}
          <div className="p-8 border-t border-slate-100 dark:border-slate-700 flex gap-3 shrink-0 bg-slate-50/80 dark:bg-slate-900/40">
              <button 
                type="button" 
                onClick={() => navigate(-1)} 
                className="flex-1 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all hover:bg-slate-100 text-slate-500"
              >
                Cancelar
              </button>
              <button 
