...
export const EstablishmentDetail: React.FC<EstablishmentDetailProps> = ({ establishments, transactions, onUpdateTransaction, settings, userRole }) => {
  ...
  return (
    <div className="relative min-h-[calc(100vh-100px)]">
      ...
      {/* Pop-up de Edição Centralizado na Viewport (fora do div animado) */}
      <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity duration-300 ${editId ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
              ...
              <div className="p-8 bg-slate-50/50 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-700 shrink-0">
                <div className="flex gap-3">
                    ...
                    <button onClick={() => {
                        ...
                        closeEdit();
                    }} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-200 dark:shadow-none transition-transform active:scale-95">Gravar Alteração</button>
                </div>
              </div>
          </div>
      </div>
