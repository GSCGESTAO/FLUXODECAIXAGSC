...
const App: React.FC = () => {
  ...
  const handleEditUser = async (oldEmail: string, email: string, role: UserRole) => {
    ...
    if (success) {
      // Forçar atualização local antes do sync para feedback imediato
  ...
  // Se houver erro de sincronização inicial e ainda não sabemos se é autorizado
  if (syncError && isAuthorized === null) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-slate-500 text-sm mt-2 max-w-xs">Não foi possível validar suas permissões com o Google Sheets.</p>
  ...
  if (isAuthorized === null) return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-6 text-center">
      ...
      <p className="text-slate-500 text-sm mt-2">Consultando base de usuários autorizados...</p>
    </div>
  );
